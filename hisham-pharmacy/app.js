    ...
    initializeProducts() {
        ...
        if (adminProducts) {
            try {
                ...
                console.warn('Failed to load admin products:', e);
            }
        }
        ...
        <div class="product-card" style="animation-delay: ${index * 0.1}s">
            <div class="product-image">
                ...
                <div class="product-badges">
                    ${product.bestseller ? '<span class="badge badge-bestseller">الأكثر مبيعاً</span>' : ''}
                    ${product.featured ? '<span class="badge badge-featured">مميز</span>' : ''}
                    ${product.discount ? `<span class="badge badge-discount">خصم ${product.discount}%</span>` : ''}
                </div>
                
                <!-- Favorite Button -->
                <button class="favorite-btn ${store.isFavorite(product.id) ? 'active' : ''}" 
                        onclick="toggleFavorite('${product.id}')" title="إضافة للمفضلة">
            ...
            <div class="product-info">
                ...
                <div class="product-actions">
                    ...
                    <button class="whatsapp-order-btn" onclick="openWhatsAppOrder('${product.id}')" title="طلب عبر الواتساب">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(() => {
        ...
        if (adminProducts) {
            try {
                const parsedProducts = JSON.parse(adminProducts);
                if (JSON.stringify(parsedProducts) !== JSON.stringify(store.products)) {
                    store.products = parsedProducts;
                    renderProducts();
                }
            } catch (e) {
                console.warn('Failed to sync admin products:', e);
            }
        }
    }, 3000);
